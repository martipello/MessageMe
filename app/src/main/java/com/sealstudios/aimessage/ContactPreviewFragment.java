...
public class ContactPreviewFragment extends DialogFragment {
    ...
    private void getContact() {
        Bundle b = new Bundle();
        b.putString(Constants.CONTACTS,contactId);
        getLoaderManager().initLoader(loader,b,dataLoaderListener);
    }
*/
    /*
    private android.support.v4.app.LoaderManager.LoaderCallbacks<DatabaseContacts> dataLoaderListener
            = new android.support.v4.app.LoaderManager.LoaderCallbacks<DatabaseContacts>() {
        @NonNull
        @Override
        ...
        @Override
        public void onLoadFinished(@NonNull Loader<DatabaseContacts> loader, DatabaseContacts data) {
    ...
    public static class userLoader extends
            AsyncTaskLoader<DatabaseContacts> {
        // You probably have something more complicated
        // than just a String. Roll with me
        private DatabaseContacts mData;
        private String userId;

        public userLoader(Context context, Bundle idBundle) {
            super(context);
            userId = idBundle.getString(Constants.CONTACTS);
        }
        ...
        @Override
        public DatabaseContacts loadInBackground() {
            LiveDbOpenHelper liveDbOpenHelper = LiveDatabaseBuilder.getUserDatabase(this.getContext());
            return liveDbOpenHelper.contactDaoLive().findById(userId).getValue();

        }

        @Override
        public void deliverResult(DatabaseContacts data) {
            mData = data;
